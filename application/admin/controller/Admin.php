...
use app\common\enum\DbIsValidEnum;
...
class Admin extends Common
{
    ...
    {
        $condition = $this->convertRequestToWhereSql();
        $condition["whereSql"] .= " and group_id = 7";
        $list = $this->adminService->getListByCondition($condition);
        $this->assign('list', $list);
    ...
    {
        return $this->fetch("addCustomerPage");
    }
    ...
    public function doAddCustomer()
    {
        ...
        try {
            ...
            $service->doAdd($username, $password, $realName, session('uid'));
        } catch (AppException $e) {
            $this->error('账号已存在');
        }

        $this->success("添加成功",url("customerList"));
    }
    ...
    public function editCustomerPage()
    {
        ...
        $info = $this->adminService->findById($id);
        $this->assign("info", $info);
        return $this->fetch("editCustomerPage");
    }
    ...
    public function doEditCustomer()
    {
        ...
        try {
            $service = new Customer();
            $service->doEditCustomer($id, $username, $password, $realName, session('uid'));
        } catch (AppException $e) {
            $this->error('账号已存在');
        }
        $this->success("修改成功",url("customerList"));
    }
    ...
    public function deleteCustomer()
    {
        ...
        try {
            $service = new Customer();
            $service->deleteCustomer($id, session('uid'));
        } catch (AppException $e) {
        ...
        $this->redirect("customerList?page=$page&keyword=$keyword");
    }
}